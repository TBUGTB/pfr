...
/-- If $G$ is an additive group and $X$ is a $G$-valued random variable and $H\leq G$ is a finite 
subgroup then, with $\pi:G\to G/H$ the natural homomorphism we have (if $U_H$ is the  uniform 
distribution on $H$, independent of $X$)
\[\mathbb{H}(\pi(X))\leq 2d[X;U_H].\] -/
lemma ent_of_proj_le [IsProbabilityMeasure μ] {UH: Ω → G} 
  (hX : Measurable X) (hU: Measurable UH) (hunif: IsUniform H UH μ) [FiniteRange X] 
  (H: AddSubgroup G) (hIndep : IndepFun X UH μ) : 
